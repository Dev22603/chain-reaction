...
## Corpus Check
- 104 files · ~99,355 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 322 nodes · 417 edges · 41 communities detected
...
- `dispatch()` --calls--> `handleMove()`  [INFERRED]
  backend\src\router.ts → backend\src\handlers\game.handlers.ts
- `dispatch()` --calls--> `handleLeaveGame()`  [INFERRED]
  backend\src\router.ts → backend\src\handlers\game.handlers.ts
- `applyMove()` --calls--> `handleMove()`  [INFERRED]
  backend\src\game\gameLogic.ts → backend\src\handlers\game.handlers.ts
- `sendGameStateToPlayer()` --calls--> `send()`  [INFERRED]
  backend\src\handlers\game.handlers.ts → backend\src\utils\broadcast.ts
...
Cohesion: 0.14
Nodes (15): broadcast(), send(), applyMove(), createBoard(), getCriticalMass(), getNeighbors(), createRoom(), getQueueKey() (+7 more)
...
Cohesion: 0.11
Nodes (5): buildGuestIdentity(), getTokenFromRequest(), handleConnection(), resolveIdentity(), wireSocketEvents()
...
Cohesion: 0.32
Nodes (13): advanceTurn(), broadcastGameState(), computeAuthDeltas(), eliminateAndBroadcast(), endGame(), getRoomForPlayer(), getWinner(), graceExpireLeaveGame() (+5 more)

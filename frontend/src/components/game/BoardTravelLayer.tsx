      ...
      <AnimatePresence>
        {travelEffects.flatMap(([key, effect]) => {
          ...
          return neighborDirections(row, col, rows, cols).map((direction) => (
            <TravelBeam
              key={`${effect.id}-${direction.rowDelta}-${direction.colDelta}`}
              left={left}
              top={top}
              color={color}
              x={direction.colDelta * cellWidth}
              y={direction.rowDelta * cellHeight}
              atomSize={atomSize}
            />
          ));
        })}
...
function effectColor(effect: BoardCellEffect): string {
  if (effect.previousOwner !== null && effect.previousOwner !== undefined) {
    return playerColor(effect.previousOwner);
  }
  if (effect.nextOwner !== null && effect.nextOwner !== undefined) {
    return playerColor(effect.nextOwner);
  }
  return "#ffffff";
}

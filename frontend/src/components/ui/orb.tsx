...
export function Orb({ color, size = 14, delay = 0, critical = false, spawn = true, className, style }: OrbProps) {
  const glow = typeof size === "number" ? size : 16;
  ...
  return (
    <span
      ...
      style={{
        ...
        boxShadow: `0 0 ${glow * 0.6}px ${color}, 0 0 ${glow * 1.4}px color-mix(in srgb, ${color} 60%, transparent), inset 0 0 ${glow * 0.3}px rgba(255,255,255,0.4)`,
        color,
        animation,

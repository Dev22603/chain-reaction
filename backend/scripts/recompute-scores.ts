...
// Only ranked matches affect scores (see src/db/repos/scores.ts applyMatchResult).
...
async function main(): Promise<void> {
  ...
  const scoreMap = new Map<
    string,
    { score: number; wins: number; losses: number; gamesPlayed: number; forfeits: number }
  >();
  ...
  for (const match of matches) {
    for (const participant of match.participants) {
      ...
      // Scoring policy mirrors src/db/repos/scores.ts applyMatchResult (lines 26-69):
  ...
  console.log(
    `recomputed ${matches.length} matches across ${scoreMap.size} players, total time ${elapsed}ms`
  );
}

...
export function getNeighbors(row: number, col: number, rows: number, cols: number): Array<[number, number]> {
	// Optimization: Avoid intermediate array allocations and .filter() to prevent garbage collection bottlenecks.
...
export function getCriticalMass(row: number, col: number, rows: number, cols: number): number {
	// Optimization: Direct mathematical boundary checks instead of array allocations via getNeighbors().length.
	let mass = 4;
	if (row === 0) mass -= 1;

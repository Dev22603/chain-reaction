...
export function getNeighbors(row: number, col: number, rows: number, cols: number): Array<[number, number]> {
	// Optimization: Avoid intermediate array allocations and filter calls in hot loops
...
export function getCriticalMass(row: number, col: number, rows: number, cols: number): number {
	// Optimization: Compute mathematically rather than allocating a neighbors array
	let mass = 4;
	if (row === 0) mass -= 1;

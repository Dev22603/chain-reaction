...
export function getNeighbors(row: number, col: number, rows: number, cols: number): Array<[number, number]> {
	// Optimization: Avoid intermediate array allocations and higher-order functions (.filter) in hot loops.
	// Use direct array mutations (.push()) to prevent garbage collection bottlenecks.
...
export function getCriticalMass(row: number, col: number, rows: number, cols: number): number {
	// Optimization: Use a primitive additive approach from 0 instead of allocating an array to count neighbors.
	// This reduces garbage collection overhead in hot loops.
	let mass = 0;
	if (row > 0) mass += 1;

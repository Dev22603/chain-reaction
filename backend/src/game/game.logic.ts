...
export function getNeighbors(row: number, col: number, rows: number, cols: number): Array<[number, number]> {
	// Optimization: Direct array mutations instead of intermediate arrays + filter
	// Reduces GC pressure in the hot loop
...
export function getCriticalMass(row: number, col: number, rows: number, cols: number): number {
	// Optimization: Mathematical bounds checking without allocating an array
	// Additive approach correctly handles edge cases like 1xN grids
	let mass = 0;
	if (row > 0) mass += 1;

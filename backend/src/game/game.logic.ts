...
export function getNeighbors(row: number, col: number, rows: number, cols: number): Array<[number, number]> {
	// Optimization: Use direct array mutations to prevent intermediate array allocations
...
export function getCriticalMass(row: number, col: number, rows: number, cols: number): number {
	// Optimization: Use additive mathematical boundary checks to avoid array allocations completely
	let mass = 0;
	if (row > 0) mass += 1;

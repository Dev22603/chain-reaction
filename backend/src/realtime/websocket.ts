	...
	const wss = new WebSocketServer({
		...
		verifyClient: (info, done) => {
			...
			if (currentIpConnections >= LIMITS.MAX_CONNECTIONS_PER_IP) {
				...
				logger.warn("websocket upgrade rejected: too many connections from IP", {
					ip: anonymizeIp(ip),
					currentIpConnections,
				});
...
function wireSocketEvents(socket: WebSocket, identity: ConnectionIdentity, ip: string): void {
	socket.on("message", (raw) => {
		...
		if (isSustainedAbuse) {
			...
			logger.warn("terminating socket due to sustained message rate abuse", {
				ip: anonymizeIp(ip),
				playerId: identity.playerId,
			});
